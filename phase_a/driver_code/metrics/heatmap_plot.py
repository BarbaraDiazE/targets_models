...
class PlotHeatmap:
    ...
    def get_models(self):
        """return all fk (id model corresponding to target)"""
        json_filename = f"models_id.json"
        json_root = f"{self.local_root}/results/trained_results/id_models"
        data = read_id_json(json_root, json_filename)
        fk_models = [key for key, value in data.items()]
        return fk_models
    ...
    def build_df(self):
        filtered_models = self.get_models()
    ...
    def plot(self):
        ...
        output_figure = f"heatmap_{self.algorithm}.png"
        plt.savefig(
